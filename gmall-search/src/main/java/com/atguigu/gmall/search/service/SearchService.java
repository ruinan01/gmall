...
public class SearchService {
    ...
    private SearchResponseVo parseSearchResult(SearchResponse response) {
        ...
        if (!CollectionUtils.isEmpty(brandBuckets)) {
            responseVo.setBrands(
                    brandBuckets.stream().map(bucket -> {
    ...
    private SearchSourceBuilder buildDsl(SearchParamVo paramVo) {
        ...
        // 1.2.5 规格参数的嵌套过滤 props=5:高通-麒麟&props=6:骁龙865-硅谷1代
        List<String> props = paramVo.getProps();
        if (!CollectionUtils.isEmpty(props)) {
            // 4:8G-12G
            props.forEach(prop -> {
                String[] attrs = StringUtils.split(prop, ":");// 以:进行分割
                if (attrs != null && attrs.length == 2) {  // [4:8G-12G,   ,xxx] :分割长度是 1 如果是空的 也不行
                    BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
                    boolQuery.must(QueryBuilders.termQuery("searchAttrs.attrId",attrs[0])); // 第一个是Id
