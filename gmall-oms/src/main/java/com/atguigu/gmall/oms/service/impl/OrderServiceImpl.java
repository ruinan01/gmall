...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, OrderEntity> implements OrderService {
    ...
    @Autowired
    private GmallPmsClient pmsClient;  // 大量远程调用 需要异步编排!!!
    ...
    public OrderEntity saveOrder(OrderSubmitVo submitVo,Long userId) {
        ...
        orderEntity.setPayAmount(submitVo.getTotalPrice()); // 支付价格
        orderEntity.setPayType(submitVo.getPayType()); // 支付类型
        orderEntity.setSourceType(0); // 订单来源 0 pc订单,1 app订单
        orderEntity.setStatus(0); // 支付状态 0 待付款 1 代发货 2....
        orderEntity.setDeliveryCompany(submitVo.getDeliveryCompany()); // 快递方式
        ...
        // 2 保存订单详情表 需要订单表的id 所以要先保存上面的订单表
